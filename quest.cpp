Черновик для ответов на вопросы по экзамену по С++ на зимней сессии первого курса Академического Университета. Для Вас старались
Недиков Константин, Орлова Александра и Sublime. Приятного чтива.
...
Также виртуальные функции могут быть объявлены с присвоением нуля. Это будет говорить компилятору о том, что они обязательно должны 
...
- scoped_ptr
	Мы не хотим вечно думать о памяти, о том, чтобы её чистить, поэтому давайте сделаем умный указатель, который сам будет
заботиться о высвобождении памяти, когда выходит из области видимости.
class scoped_ptr
{
	private:
		Person *p;
...
if() {
	...
	*p.hasBirthday();
	...
	scoped_ptr p1 = p; в конце дважды деструктор вызовется;
...
Реализация всего лежит тут: "https://dpaste.de/PJiQ", там не мало. Но для ответа на билет достаточно будет описать идею
...
оператор [] тоже имеет особенности. Его следует дважды, с const и без.
int* operator[](std::size_t i);
const int* operator[](std::size_t i) const;
Это нужно из-за разницы в присваивании и просто взятии значения. Если у нас указатель на константный объект, мы не сможем
взять по нему значения первым способом.

"https://dpaste.de/ybpU" пример с практики.
...
BigInt a = (BigInt)3;
	...
	|		 v
 a(tmp)<----tmp
Современные компиляторы могут это сделать быстрее, без создания третьей переменной.
...
в 2.cpp extern int c; Тогда компилятор не будет создавать переменную, но он будет знать, что такая существует.
...
Все такие вещи обрабатываются на стадии линковки, есть 2 линковки - внутренняя(internal) и внешняя(exteral). Сначала линковщик
...
Мы создали переменную один раз, а дальше при вызове функции она не будет создаваться заново, она будет увеличивать старую переменную,
при этом область видимости у неё всё ещё локальная. При этом проинициализируется она переменной k только один раз, остальные будет всё равно,
...
public:
	...
	Это будет переменная глобальная для всех объектов это класса, но область видимости будет только внутри него.
...
Но мы не хотим, чтобы её изменяли. Тогда запишем эту переменную в private модификатор доступа. Вроде всё хорошо, но вот
...
- inline у функций
inline не обещает, что функция будет встроена в код, компилятор сам решит, стоит ли это делать и сможет ли он это сделать.
Также инлайн функциям нужно небольшое ограничение - они должны знать тело. Если у нас тело функции будет в одном месте, а
при вызове мы будем знать лишь объявление - компилятор не поймёт, что ему нужно делать, но если в каждом файле будут одинаковые
функции - линковщик сойдёт сума. Для этого для инлайн функций сделано исключение, линковщик выбирает какую-то одну из тех,
что одинаковые, а другие реализации просто игнорирует. Так что это приведёт к undefined behavior, если сделать разные реализации
инлайн функций с одинаковым интерфейсом.
Также все функции, определённые внутри класса в header файле по умолчанию являются inline функциями. Чтобы они не были инлайн -
нужно написать определение отдельно. 
...
- Автотесты
	Чтобы избежать появления ошибок во время изменения программы, особенно, когда над программой работает сразу несколько человек,
люди придумали договорённость писать автотесты - набор функций, которые вызывают функции из программы и проверяют, что результат
соответствует ожиданию. Притом тесты делают это автоматически, на выходе лишь говоря, что всё прошло успешно или нет, и где оно
споткнулось, чтобы программисту не терять время на ввод тестов руками. 
	Существуют пару религиозных фанатизмов по тестам - это unit тесты и test driven код. Первое - это идеология того, что тесты
должны быть у каждой программы и должны полностью покрывать все строки кода, каждую функцию программы. Второе ещё более странное,
это когда люди пишут сначала тесты, т.е. ожидаемую работу программы, а уже потом пишут сам код и реализации этих функций.
	Автотесты также являются хорошей документацией программы, ибо они отчётливо показывают поведение функций и ожидаемый от них
результат. 

- Декомпозиция программы (Model, View)
	Есть разные шаблоны программирования, например, описанные в одной популярной книжке Design patterns. Бездумное использование
шаблонов может привести к бессмысленному усложнению программы. Декомпозиция программы - разбиение её на разные более простые
и маленькие блоки, пусть и связанные. Связь может быть очень спутанной между блоками, но в основном стараются, чтобы зависимости
были попроще. Например, если мы пишем крестики-нолики - можно разделить всё на model - наше игровое поле, на котором будет
просчитываться логика и храниться результаты, а также на View - графическое отображение игры. При этом Model не обязательно знать,
что происходит с View, зато View нужно работать с доской. Model получается более универсальным, мы для одного Model можем написать
разные, зависящие от него, блоки, отвечающие за вывод игры на компьютере, телефоне, в консоли или с графическим интерфейсом. 

- Ключевые слова const, enum
const - это ключевое слово означает, что данные не будут изменяться.
const int N = 10; означает, что переменную менять нельзя. 
const делает неизменяемым то, после чего стоит. Можно в некоторых случаях, как выше, менять порядок написания, но в некоторых
случаях он будет важен.
char const *s сделает указатель на char, данные которые менять нельзя. const char* s будет делать то же самое.
...
char ss[10] - массив, у него нельзя делать адресную арифметику, т.е. это аналогично такому описанию char * const ss;
Для чего же это? Во-первых, это позволяет подсказать программисту, какие поля могут изменяться, а какие точно не будут, а во-вторых,
это защита от самого себя, чтобы ничего не испортить.
Ссылки тоже константы по указателю.
	Если сделать константное поле в классе, то ему нельзя будет присвоить какое-то значение в конструкторе, но для этого идеально
подойдёт список инициализации.
	const у параметра, например, конструктор копий. Array(const Array& a){...} это предостерегает от изменения того, что меняться не должно
по логике. Так же, как и в операторах.
	У возвращаемого значения. const Array& operator=(const Array& a){...}. Этот конст защищает нас от ... чего? Ну всё не так уж
и прозрачно, вообще бред, если честно, но может когда-то это вас защитит. const у возвращаемого значения говорит, что значение только
rvalue, т.е. оно находится в правой части выражения. Ну например, мы можем написать так: a = (b = c), но не можем написать так:
(a = b) = c, что по сути своей логично, но кажется очень надуманной ситуацией
	У метода. int getSize() const; Позволяет защитить нас от изменения полей классов.

Теперь поговорим о такой прекрасной вещи, как enum, которая в плюсах реализована так себе и её все ненавидят. Это энумерации - 
списки значений, у которых есть имена. Проблемы заключаются сразу в том, что значения эти только целочисленные и это глобальные переменные.
enum key_code{shift_code, z_code, x_code = 10, c_code, v_code}. Мы можем делать переменные, которые должны будут принимать по логике 
только эти значения. Здесь у shift_code будет 0, у z_code 1, у x понятно, дальше 11 и 12. Почему так? Ну каждому последующему даётся
значение на 1 больше, а также можно задавать значения вручную. Проблемы у этого такие же, как и глобальных переменных, а также могут
получиться одинаковые значения у нескольких переменных, мы ожидаем, что получив z_code пользователь нажал на z, а на самом деле он мог
нажать на f, у которой мы случайно указали такой же код. Плюс чтобы сделать значения не интовые, а строковые - в дело подключаются костыли
всемогущие. Нам практик показывал, как с помощью макросов и генераторов получилось сделать более менее адекватное подобие энумерации со
строковыми значениями, но это вышел довольно адский и плохо читаемый код. В c++ появились также enum классы, но о них мы формально не
говорили. Значения в энумерациях, если я не ошибаюсь, это long long или что-то больше, лень гуглить.
