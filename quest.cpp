...
другим файлам, в которых может использоваться эта функция, соответсвенно эти файлы должны быть перекомпилированны или переписаны, если,
например, изменили возвращаемое значение функции. Заголовочные файлы подключаются коммандой для препроцессора #include headername,
...
который редактирует код, заменяет некоторые конструкции на более понятные для машины, выполняет все комманды препроцессора, 
например #include или #define. Далее идёт трансляция в ассемблерный код, \??\из него\??\ в объектные файлы. Далее на стадии линковки
связываются все имена переменых и функций с физическими адресами машины. На выходе получаем executable файл.

- утилита make
	make служит для упрощённой сборки проектов. Особенно полезен, когда в проекте много файлов. Он построем на простом принципе, есть цели
и их зависимости. Если зависимости изменились, то нужно изменить цель - следовательно вызываются комманды, написанные ниже. Важной
особенностью является то, что комманды нужно писать с отступом с помощью tab. Выглядит это как-то так:
<цель>: <зависимости>
	<комманды>
порядок записей в makefile тоже важен. По умолчанию выполняется первая цель, поэтому обычно её делают на подобии такого
all: bin main 
Чтобы вызвались соответсвующие зависимости, которые являются целями ниже. Чтобы использовать make, достаточно написать в консоли make,
...
- применение указателей и ссылок
	Указатель на перменную - это номер ячейки памяти, её адрес, если быть точнее, в котором хранятся данные переменной. Если переменная
...
уже занята под что-то другое, куда лезть не стоило. Можно делать укзаталь на указатель int**, или брать значение, лежашее в памяти, на
...
а также избавиться от звёздочек, для работы со значениями. При комплияции это просто заменяется на указатели и разименование(как отличать ссылки
...
С указателями можно работать, как с обычными интовыми перменными, операторы +, -, ++ и т.п., при этом просто адрес сдвинется
...
значениями не имеют указателей друг на друга. Интризивный список позволяет создать лишь одну реализацию списка, а потом можно делать
различные блоки со значениями, различными типами и функциями, которые будут работать поверх одного интризивного списка.	
...
container_of(указатель на интрузивную ноду, тип который её содержит, название поля с нашей интризивной нодой), а возвращает это
...
Эта штука копирует в destination num байтов из sorce.
...
то же самое, но для строк. Если одна строка короче другой - сравнение будет соответсвенно <0 при str1 короче.

char* strcat (char* destination, const char* source);
Конкатинирует строки. Приклеивает source к destination справа. Возвращает destination.

char* strstr (const char* str1, const char* str2);
Вовзращает указатель на начало второй строки, если она находится в первой. Например strstr(str, "cpp");/*str = "it's a cpp, baby";*/
в этой строке найдётся cpp и вернётся указатель на символ 'c' в этой строке. Если его не нашлось - возращается нолевой указатель.

char* strchr (const char* str, int character);
та же муть, но находит символ в строке.


- stdlib.h (malloc/free, calloc, realloc, atoi, strtoll, srand/rand, qsort) //<=================== 2 пукнт
void* malloc (size_t size);
первый гость нашей программы, выделяет на куче память такого размера и возвращает указатель на начало выделнного блока
...
инициализирует гениратор рандома по сиду. Обычно подклюается time.h и пихают в качестве сида время.
...
не хочется, ведь всё остальное можно оставить - ну тогда просто можно унаследоваься от класса и менять только необходимое.
	Плюс появилась перегрузка, можно было написать класс, который работал бы по-разному с разными типами, а также можно было бы
унаследоваться и перекрыть старые функции, изменив интерфейс класса, к примеру, помеянть один блок на другой - это полиморфизм.
...
умолчанию выбрать свои. Если передан один параметр - создаст вектор такого размера, если два - создаст и проинциализирует. С++ может,
...
т.п., то можно воспользоваться стандарной реализацией и просто не писать конструктор. Деструктор же нужен, чтобы убить всё то, что ты
